...
{
    public class TipoRespuestaBL : Base
    {
        public IList<TipoRespuestaDTO> getTiposRespuesta(bool activeOnly = false)
        {
            using (var context = getContext())
            {
                var result = !activeOnly ? context.TipoRespuesta
                    .Select(r => new TipoRespuestaDTO { IdTipoRespuesta = r.IdTipoRespuesta, Nombre = r.Nombre, Estado = r.Estado })
                    : context.TipoRespuesta.Where(r => r.Estado == true)
                    .Select(r => new TipoRespuestaDTO { IdTipoRespuesta = r.IdTipoRespuesta, Nombre = r.Nombre, Estado = r.Estado });
